namespace Foo
{
  ...
  public class Program
  {
      ...
      foreach(string line in input)
      {
        ...
        foreach(var n in numbers)
        {
          ...
          foreach(var d in div)
          {
            ...
            {
              Console.WriteLine("{0} / {1} = {2}", d, number, d / number);
              
              sum += d / number;
            }
            else if ((number / d) * d == number)
            {
              Console.WriteLine("{0} / {1} = {2}", number, d, number / d);
    ...
    private static string[] inputF = new string[] {
